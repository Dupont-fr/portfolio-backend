...
async function callGeminiImage(prompt: string, aspectRatio = '16:9'): Promise<{ mimeType: string; data: string }> {
  const apiKey = env.aiApiKey
  if (!apiKey) {
    throw new ApiError(503, 'Clé API Gemini non configurée sur le serveur (AI_API_KEY).')
  }
    ...
    for (const withAspect of attempts) {
      try {
        ...
        if (!response.ok) {
          const detail = await response.text().catch(() => '')
          console.error(
            `[ai-image] ${model} (aspect=${withAspect}) : ${response.status}`,
            detail.slice(0, 200),
          )
          if (response.status === 429) {
...
interface ArticleImagesResult {
  coverUrl: string | null
  content: string | null
}

async function generateArticleImages(topic: string, content: string): Promise<ArticleImagesResult> {
  ...
  const tasks: { kind: 'cover' | 'section'; heading?: string; prompt: string }[] = [
    {
      kind: 'cover',
    ...
    tasks.map((task, index) =>
      (async () => {
        ...
        const url = await uploadImageToCloudinary(
          image.data,
          image.mimeType,
          `portfolio/ai/blog/${slug || 'article'}-${timestamp}-${index + 1}`,
        )
        return { task, url }
      })(),
    ),
